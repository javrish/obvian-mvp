...
class P3NetPipelineIntegrationTest {
  ...
  class EndToEndPipelineTests {
    ...
    void completeDevOpsPipelineProcessing() throws Exception {
      ...
      Map<String, Object> parseResponse =
          objectMapper.readValue(parseResult.getResponse().getContentAsString(), Map.class);
      ...
      Map<String, Object> buildResponse =
          objectMapper.readValue(buildResult.getResponse().getContentAsString(), Map.class);
      ...
      List<Map<String, Object>> transitions =
          (List<Map<String, Object>>) petriNetData.get("transitions");
      ...
      Map<String, Object> validateResponse =
          objectMapper.readValue(validateResult.getResponse().getContentAsString(), Map.class);
      ...
      Map<String, Object> simulateResponse =
          objectMapper.readValue(simulateResult.getResponse().getContentAsString(), Map.class);
      ...
      Map<String, Object> dagResponse =
          objectMapper.readValue(dagResult.getResponse().getContentAsString(), Map.class);
    ...
    void completeFootballTrainingPipelineProcessing() throws Exception {
      ...
      assertThat(places)
          .hasSizeGreaterThanOrEqualTo(6); // warmup → fork → parallel branches → join → cooldown
      ...
      assertThat(trace.size())
          .isGreaterThanOrEqualTo(4); // At least: warmup, fork, pass/shoot, join, cooldown

      // Step 5: Project to DAG preserving parallel semantics
      PetriDagRequest dagRequest = new PetriDagRequest();
      dagRequest.setPetriNet(convertMapToPetriNet(petriNetData));
      dagRequest.setProjectionOptions(
          Map.of(
      ...
      System.out.printf(
          "✅ Football E2E Pipeline completed in %dms%n", totalExecutionTime.toMillis());
    }
  ...
  class ComponentIntegrationTests {
    ...
    void automationGrammarToPetriNetValidatorIntegration() throws Exception {
      ...
      String complexWorkflow =
          "initialize data, then process A and process B in parallel, then aggregate results,"
      ...
      PetriBuildRequest buildRequest = new PetriBuildRequest();
      buildRequest.setIntent(
          convertMapToPetriIntentSpec((Map<String, Object>) parseResponse.get("intent")));
      ...
      PetriValidateRequest validateRequest = new PetriValidateRequest();
      validateRequest.setPetriNet(
          convertMapToPetriNet((Map<String, Object>) buildResponse.get("petriNet")));
    ...
    void petriTokenSimulatorToPetriToDagProjectorIntegration() throws Exception {
          ...
          mockMvc
              ...
              .andDo(
                  result -> {
                    ...
                    buildReq.setIntent(
                        convertMapToPetriIntentSpec((Map<String, Object>) response.get("intent")));
      ...
      Map<String, Object> petriNetData =
          (Map<String, Object>) goldenFootballResults.get("petriNet");
  ...
  class ApiIntegrationTests {
    ...
    void allFivePetriControllerEndpointsWithRealProcessing() throws Exception {
      String testWorkflow =
          "authenticate user, then load dashboard and check notifications in parallel, then display"
              + " results";
    ...
    void performanceTestingSub2sProcessingTarget() throws Exception {
            ...
            CompletableFuture.runAsync(
                () -> {
                  try {
                    ...
                    String workflow =
                        String.format(
                            "process request %d, validate data %d, send response %d",
                    ...
                    throw new RuntimeException(
                        "Performance test failed for request " + requestId, e);
                  }
                });
      }
      ...
      OptionalDouble averageTime =
          pipelineExecutionTimes.stream().mapToLong(Duration::toMillis).average();
      ...
      System.out.printf(
          "✅ Performance test completed - Average: %.2fms, Requests: %d%n",
    ...
    void crossHighlightingMetadataValidation() throws Exception {
      String workflowWithMetadata =
          "initialize system with config A, then process data with algorithm B, finally output"
              + " results with format C";

      PetriParseRequest parseRequest = new PetriParseRequest();
      parseRequest.setText(workflowWithMetadata);
      parseRequest.setTemplateHint("data-pipeline");
      parseRequest.setMetadata(
          Map.of(
              "highlightFeatures",
              Arrays.asList("config", "algorithm", "format"),
      ...
      if (!hasElementMetadata) {
        for (Map<String, Object> transition : transitions) {
          if (transition.containsKey("metadata")
              && !((Map<?, ?>) transition.get("metadata")).isEmpty()) {
            hasElementMetadata = true;
            break;
          }
        }
    ...
    void healthCheckEndpointValidation() throws Exception {
      mockMvc
          .perform(get("/api/v1/petri/health"))
          .andExpect(status().isOk())
  ...
  class GoldenTestOutputsAndRegressionTesting {
    ...
    void generateAndValidateGoldenDevOpsOutputs() {
      ...
      List<Map<String, Object>> transitions =
          (List<Map<String, Object>>) petriNet.get("transitions");
    ...
    void regressionTestingAgainstGoldenOutputs() {
      ...
      if (goldenDevOpsResults.containsKey("petriNet")) {
        Map<String, Object> goldenPetriNet =
            (Map<String, Object>) goldenDevOpsResults.get("petriNet");
  ...
  class PerformanceBenchmarksAndCoverageAnalysis {
    ...
    void pipelinePerformanceBenchmarks() {
      ...
      long maxTime = pipelineExecutionTimes.stream().mapToLong(Duration::toMillis).max().orElse(0L);

      long minTime = pipelineExecutionTimes.stream().mapToLong(Duration::toMillis).min().orElse(0L);
      ...
      System.out.printf(
          "✅ Performance benchmarks - Avg: %.2fms, Min: %dms, Max: %dms, Samples: %d%n",
    ...
    void componentCoverageAnalysis() {
      ...
      System.out.printf(
          "✅ Coverage analysis - Components: %d, Critical Flows: %d%n",
    ...
    void memoryUsageAnalysis() {
      ...
      System.out.printf(
          "✅ Memory analysis - Used: %.2fMB, Total: %.2fMB, Utilization: %.1f%%%n",
          usedMemory / 1024.0 / 1024.0, maxMemory / 1024.0 / 1024.0, memoryUtilization * 100);
    }
  ...
  private PetriNet createBrokenPetriNet() {
    ...
    Transition orphanTransition =
        new Transition("t_orphan", "Orphan Transition", "Disconnected transition");
    ...
    return Map.of(
        ...
        "places",
            Arrays.asList(
                ...
                Map.of(
                    "id", "p_warmup", "name", "After Warmup", "description", "After warmup phase"),
