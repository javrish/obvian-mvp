...
class P3NetPerformanceBenchmarkTest {
  ...
  class ComponentLatencyBenchmarks {
    ...
    void automationGrammarTransformationLatencyBenchmark() throws Exception {
      ...
      for (PetriIntentSpec intent : testWorkflows) {
        ...
        assertThat(averageTime.toMillis())
            .describedAs(
                "AutomationGrammar average latency for %d-step workflow", intent.getSteps().size())
            .isLessThan(COMPONENT_LATENCY_TARGET_MS);

        System.out.printf(
            "✅ AutomationGrammar %d steps: avg=%dms, min=%dms, max=%dms%n",
            intent.getSteps().size(),
    ...
    void petriNetValidatorValidationLatencyBenchmark() throws Exception {
      ...
      for (PetriNet petriNet : testPetriNets) {
        ...
        assertThat(averageTime.toMillis())
            .describedAs(
                "PetriNetValidator average latency for %d-place PetriNet",
                petriNet.getPlaces().size())
            .isLessThan(COMPONENT_LATENCY_TARGET_MS);

        System.out.printf(
            "✅ PetriNetValidator %d places: avg=%dms%n",
    ...
    void petriTokenSimulatorSimulationLatencyBenchmark() throws Exception {
      ...
      for (PetriNet petriNet : testPetriNets) {
        ...
        assertThat(averageTime.toMillis())
            .describedAs(
                "PetriTokenSimulator average latency for %d-transition PetriNet",
                petriNet.getTransitions().size())
            .isLessThan(COMPONENT_LATENCY_TARGET_MS);

        System.out.printf(
            "✅ PetriTokenSimulator %d transitions: avg=%dms%n",
      ...
      for (String prompt : testPrompts) {
        ...
        for (int run = 0; run < 5; run++) {
          ...
          PetriNetValidationResult validationResult =
              petriNetValidator.validate(petriNet, validationConfig);
        ...
        Duration averagePipelineTime =
            Duration.ofMillis(
                pipelineRunTimes.stream().mapToLong(Duration::toMillis).sum()
                    / pipelineRunTimes.size());
  ...
  class ThroughputBenchmarks {
    ...
    void concurrentPipelineProcessingThroughput() throws Exception {
      ...
      for (int i = 0; i < concurrentWorkflows; i++) {
        ...
        CompletableFuture<Duration> future =
    ...
    void componentThroughputUnderSustainedLoad() throws Exception {
      ...
      double validatorThroughput =
          (double) sustainedOperations / validatorSustainedTime.toMillis() * 1000.0;

      System.out.printf("✅ AutomationGrammar sustained throughput: %.2f ops/sec%n", agThroughput);
      System.out.printf(
          "✅ PetriNetValidator sustained throughput: %.2f ops/sec%n", validatorThroughput);
    ...
    void memoryUsagePerWorkflowProcessing() throws Exception {
      ...
      for (PetriIntentSpec intent : testWorkflows) {
        ...
        assertThat(workflowMemoryMB)
            .describedAs(
                "Memory usage for %d-step workflow should be reasonable", intent.getSteps().size())
            .isLessThan(MEMORY_TARGET_MB);

        System.out.printf(
            "✅ Memory usage %d steps: %dMB%n", intent.getSteps().size(), workflowMemoryMB);
    ...
    void memoryUsageUnderSustainedLoad() throws Exception {
      ...
      for (int i = 0; i < 50; i++) {
        ...
        if (i % 10 == 0) {
          ...
          System.out.printf(
              "Memory after %d workflows: %dMB%n",
  ...
  private void performWarmup() {
    ...
    try {
      ...
      SimulationConfig warmupSimConfig =
          SimulationConfig.builder()
              .mode(SimulationConfig.Mode.DETERMINISTIC)
              .maxSteps(5)
              .stepDelayMs(0)
  ...
  private PetriIntentSpec createComplexWorkflow() {
    return PetriIntentSpec.builder()
        ...
        .originalPrompt(
            "init, validate; if ok then process A and B in parallel and notify; if not ok then"
                + " cleanup")
        .templateId("complex")
        .addActionStep("init", "Initialize")
        ...
        .addStep(
            new PetriIntentSpec.IntentStep(
                ...
                Arrays.asList("validation_choice"),
                new HashMap<>(),
                "valid",
                new HashMap<>()))
        .addStep(
            new PetriIntentSpec.IntentStep(
                ...
                Arrays.asList("parallel_processing"),
                new HashMap<>(),
                null,
                new HashMap<>()))
    ...
    performanceMetrics.forEach(
        (component, times) -> {
          if (!times.isEmpty()) {
            ...
            System.out.printf(
                "📈 %s: avg=%.1fms, min=%dms, max=%dms, samples=%d%n",
