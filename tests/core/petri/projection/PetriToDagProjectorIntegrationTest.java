...
class PetriToDagProjectorIntegrationTest {
  ...
  void shouldIntegrateWithValidationPipeline() {
    ...
    PetriNetValidationResult validationResult =
        validator.validate(petriNet, PetriNetValidationResult.ValidationConfig.defaultConfig());
  ...
  void shouldHandleValidationFailuresGracefully() {
    ...
    PetriNet invalidNet =
        PetriNet.builder()
            ...
            .addTransition(
                new Transition("duplicate", "First", null, "action1", null, new HashMap<>()))
            .addTransition(
                new Transition("duplicate", "Second", null, "action2", null, new HashMap<>()))
            .build();

    // Validation should fail
    PetriNetValidationResult validationResult =
        validator.validate(invalidNet, PetriNetValidationResult.ValidationConfig.defaultConfig());
  ...
  void shouldIntegrateWithSimulationAndMaintainExecutionSemantics() {
    ...
    PetriNetValidationResult validationResult =
        validator.validate(
            parallelWorkflow, PetriNetValidationResult.ValidationConfig.defaultConfig());
    assertThat(validationResult.isValid()).isTrue();

    // Simulate the Petri net to understand execution behavior
    SimulationConfig config =
        SimulationConfig.builder().maxSteps(100).enableTracing(true).enableAnimation(true).build();
  ...
  void shouldHandleComplexWorkflowWithChoiceAndJoinPatterns() {
    ...
    // Validate first
    PetriNetValidationResult validationResult =
        validator.validate(complexNet, PetriNetValidationResult.ValidationConfig.defaultConfig());
    assertThat(validationResult.isValid()).isTrue();
  ...
  void shouldMaintainExecutionOrderInvariantsAfterProjection() {
    ...
    SimulationConfig config =
        SimulationConfig.builder().maxSteps(50).enableTracing(true).enableAnimation(false).build();
  ...
  private PetriNet createDevOpsPipeline() {
    return PetriNet.builder()
        ...
        .addTransition(
            new Transition("build", "Build App", null, "docker_build", null, new HashMap<>()))
        .addTransition(new Transition("test", "Run Tests", null, "pytest", null, new HashMap<>()))
        .addTransition(
            new Transition("deploy", "Deploy to Prod", null, "k8s_deploy", null, new HashMap<>()))
        .addTransition(
            new Transition(
                "notify", "Send Notification", null, "slack_notify", null, new HashMap<>()))
    ...
    return PetriNet.builder()
        ...
        .addTransition(
            Transition.builder("start")
                .name("Start Parallel")
                .action("init_parallel")
                .asFork()
                .build())
        .addTransition(
            new Transition("task1", "Execute Task 1", null, "task1_exec", null, new HashMap<>()))
        .addTransition(
            new Transition("task2", "Execute Task 2", null, "task2_exec", null, new HashMap<>()))
        .addTransition(
            Transition.builder("join").name("Join Results").action("join_results").asJoin().build())
  ...
  private PetriNet createFootballWorkflow() {
    return PetriNet.builder()
        ...
        .addTransition(
            new Transition("scout", "Scout Opponent", null, "analysis", null, new HashMap<>()))
        .addTransition(
            new Transition(
                "plan", "Plan Tactics", null, "tactical_planning", null, new HashMap<>()))
        .addTransition(
            new Transition("train", "Train Team", null, "training", null, new HashMap<>()))
        .addTransition(
            new Transition("match", "Play Match", null, "match_execution", null, new HashMap<>()))
  ...
  private PetriNet createComplexWorkflowWithChoicesAndJoins() {
    return PetriNet.builder()
        ...
        .addTransition(
            new Transition("review", "Review PR", null, "code_review", null, new HashMap<>()))
        .addTransition(
            Transition.builder("approve")
                .name("Approve PR")
                .action("approve")
                .asChoice("decision == 'approve'")
                .build())
        .addTransition(
            Transition.builder("reject")
                .name("Reject PR")
                .action("reject")
                .asChoice("decision == 'reject'")
                .build())
        .addTransition(new Transition("merge", "Merge Code", null, "merge", null, new HashMap<>()))
        .addTransition(
            new Transition("rollback", "Rollback Changes", null, "rollback", null, new HashMap<>()))
        .addTransition(
            new Transition("notify", "Send Notification", null, "notify", null, new HashMap<>()))
  ...
  private PetriNet createSequentialWorkflow() {
    return PetriNet.builder()
        ...
        .addTransition(
            new Transition("init", "Initialize System", null, "system_init", null, new HashMap<>()))
        .addTransition(
            new Transition(
                "process", "Process Data", null, "data_processing", null, new HashMap<>()))
        .addTransition(
            new Transition(
                "validate", "Validate Results", null, "data_validation", null, new HashMap<>()))
        .addTransition(
            new Transition(
                "finalize", "Finalize Process", null, "finalization", null, new HashMap<>()))
