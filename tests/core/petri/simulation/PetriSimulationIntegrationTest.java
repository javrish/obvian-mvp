...
class PetriSimulationIntegrationTest {
  ...
  class DevOpsWorkflowSimulation {
    ...
    void shouldSimulateCompleteDevOpsCiCdPipeline() {
      ...
      SimulationConfig config =
          SimulationConfig.builder().seed(42L).maxSteps(20).verbose(true).build();
      ...
      List<String> transitions = trace.stream().map(TraceEvent::getTransition).toList();
    ...
    void shouldHandleTestFailuresAndRetryLogic() {
      ...
      SimulationConfig config =
          SimulationConfig.forTesting(999L); // Different seed for different behavior
      ...
      List<TraceEvent> trace = result.getTrace();
      long testTransitions =
          trace.stream().map(TraceEvent::getTransition).filter(t -> t.contains("test")).count();
    ...
    private PetriNet createDevOpsWorkflow() {
      return PetriNet.builder()
          ...
          .addTransition(
              Transition.builder("deploy_production").name("Deploy to Production").build())
  ...
  class ChoiceWorkflowSimulation {
    ...
    private PetriNet createChoiceWorkflow() {
      return PetriNet.builder()
          ...
          .addTransition(
              Transition.builder("choose_path_a")
                  .name("Choose Path A")
                  .asChoice("condition_a")
                  .build())
          .addTransition(
              Transition.builder("choose_path_b")
                  .name("Choose Path B")
                  .asChoice("condition_b")
                  .build())
          .addTransition(new Transition("task_a", "Execute Task A"))
          .addTransition(new Transition("task_b", "Execute Task B"))
  ...
  class ResourceManagementSimulation {
    ...
    void shouldSimulateResourceConstraintsAndQueuing() {
      ...
      SimulationConfig config = SimulationConfig.builder().seed(789L).maxSteps(50).build();
  ...
  class DeterministicExecutionTests {
    ...
    void shouldProduceIdenticalExecutionTracesWithSameSeed() {
      ...
      assertThat(result1.getFinalMarking().getTokens())
          .isEqualTo(result2.getFinalMarking().getTokens());
    ...
    void shouldProduceDifferentButValidExecutionsWithDifferentSeeds() {
      ...
      List<String> transitions1 =
          result1.getTrace().stream().map(TraceEvent::getTransition).toList();
      List<String> transitions2 =
          result2.getTrace().stream().map(TraceEvent::getTransition).toList();
  ...
  class PerformanceAndScalabilityTests {
    ...
    void shouldHandleModerateSizedNetworksEfficiently() {
      ...
      SimulationConfig config =
          SimulationConfig.builder().seed(42L).maxSteps(100).enableTracing(true).build();
    ...
    private PetriNet createModerateNetwork() {
      PetriNet.Builder builder = PetriNet.builder().name("Moderate Complexity Network");
      ...
      builder
          ...
          .addArc("t1", "p2")
          .addArc("p1", "t2")
          .addArc("t2", "p3") // Parallel branch
          .addArc("p2", "t3")
          .addArc("t3", "p4")
          .addArc("p3", "t4")
          .addArc("t4", "p5")
          .addArc("p4", "t5")
          .addArc("t5", "p6") // Merge point
          .addArc("p5", "t5")
          .addArc("p6", "t6")
          .addArc("t6", "p7")
          .addArc("p7", "t7")
          .addArc("t7", "p8")
          .addArc("p7", "t8")
          .addArc("t8", "p9") // Another parallel section
          .addArc("p8", "t9")
          .addArc("t9", "p10")
          .addArc("p9", "t10")
          .addArc("t10", "p11")
          .addArc("p10", "t11")
          .addArc("t11", "p12") // Final merge
          .addArc("p11", "t11")
