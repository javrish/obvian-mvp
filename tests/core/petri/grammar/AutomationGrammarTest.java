...
class AutomationGrammarTest {
    ...
    class BasicTransformationPatterns {
        ...
        void transformSingleActionStep() throws Exception {
            ...
            assertThat(places).extracting(Place::getId)
                    .allMatch(id -> id.contains("action1"));
        ...
        void transformSequentialActionSteps() throws Exception {
            ...
            PetriIntentSpec.IntentStep action2 = new PetriIntentSpec.IntentStep(
                    "action2", PetriIntentSpec.StepType.ACTION, "Second task",
                    Arrays.asList("action1"), new HashMap<>(), null, new HashMap<>());

            PetriIntentSpec sequentialSpec = PetriIntentSpec.builder()
                    .name("Sequential Actions")
                    .description("Two actions in sequence")
                    .addActionStep("action1", "First task")
                    .addStep(action2)
            ...
            assertThat(petriNet.getPlaces()).hasSizeGreaterThanOrEqualTo(3); // At least pre1, post1/pre2, post2
        ...
        void transformChoiceStep() throws Exception {
            ...
            PetriIntentSpec.IntentStep choiceStep = new PetriIntentSpec.IntentStep(
                    "choice1", PetriIntentSpec.StepType.CHOICE, "Make a choice",
        ...
        void transformParallelStep() throws Exception {
            ...
            PetriIntentSpec.IntentStep parallelStep = new PetriIntentSpec.IntentStep(
                    "parallel1", PetriIntentSpec.StepType.PARALLEL, "Execute in parallel",
            ...
            assertThat(petriNet.getPlaces()).hasSizeGreaterThanOrEqualTo(3); // Pre-place + 2 branch places
        ...
        void transformSyncStep() throws Exception {
            ...
            PetriIntentSpec.IntentStep syncStep = new PetriIntentSpec.IntentStep(
                    "sync1", PetriIntentSpec.StepType.SYNC, "Synchronize branches",
            ...
            Optional<Transition> joinTransition = petriNet.getTransitions().stream()
    ...
    class ComplexScenarioTests {
        ...
        void transformDevOpsScenario() throws Exception {
            ...
            List<Transition> choiceTransitions = petriNet.getTransitions().stream()
                    .filter(Transition::isChoice)
                    .toList();
        ...
        void transformFootballScenario() throws Exception {
            ...
            List<Transition> forkTransitions = petriNet.getTransitions().stream()
                    .filter(Transition::isFork)
                    .toList();
            assertThat(forkTransitions).hasSizeGreaterThanOrEqualTo(1);

            // Verify join transition exists (for sync)
            List<Transition> joinTransitions = petriNet.getTransitions().stream()
                    .filter(Transition::isJoin)
                    .toList();
    ...
    class ValidationAndErrorHandling {
        ...
        void transformInvalidIntentSpecThrowsException() {
            ...
            PetriIntentSpec invalidSpec = PetriIntentSpec.builder()
        ...
        void transformEmptyIntentSpecThrowsException() {
            ...
            PetriIntentSpec emptySpec = PetriIntentSpec.builder()
                    .name("Empty Spec")
        ...
        void validateTransformabilityDetectsInvalidSpecs() {
            ...
            PetriIntentSpec.IntentStep invalidStep = new PetriIntentSpec.IntentStep(
                    "invalid", null, "Invalid step",
                    new ArrayList<>(), new HashMap<>(), null, new HashMap<>()
            );
            ...
            AutomationGrammar.ValidationResult result = automationGrammar.validateTransformability(invalidSpec);
        ...
        void validateTransformabilityAcceptsValidSpecs() {
            // Given: Valid spec
            PetriIntentSpec validSpec = PetriIntentSpec.builder()
                    .name("Valid Spec")
            ...
            // When: Validate transformability
            AutomationGrammar.ValidationResult result = automationGrammar.validateTransformability(validSpec);
    ...
    class ConfigurationAndOptimization {
        ...
        void transformWithCustomConfigurationAppliesSettings() throws Exception {
            ...
            PetriIntentSpec intentSpec = PetriIntentSpec.builder()
                    .name("Config Test")
                    .addActionStep("action1", "Test action with long description that should be truncated in descriptive mode")
                    .build();
        ...
        void getTransformationStatsProvideAccurateMetrics() {
            ...
            // When: Get stats
            AutomationGrammar.TransformationStats stats = automationGrammar.getTransformationStats(intentSpec);
    ...
    class GoldenTestOutputs {
        ...
        void simpleActionTransformationMatchesGolden() throws Exception {
            ...
            List<String> placeIds = petriNet.getPlaces().stream()
                    .map(Place::getId)
                    .sorted()
                    .toList();
    ...
    private PetriIntentSpec createDevOpsIntentSpec() {
        return PetriIntentSpec.builder()
                ...
                .addStep(new PetriIntentSpec.IntentStep(
                        "test_choice", PetriIntentSpec.StepType.CHOICE, "Check test results",
                        Arrays.asList("run_tests"), new HashMap<>(), null,
                        Map.of("paths", Arrays.asList("tests_passed", "tests_failed"))
                ))
                .addStep(new PetriIntentSpec.IntentStep(
                        "deploy", PetriIntentSpec.StepType.ACTION, "Deploy to production",
                        Arrays.asList("test_choice"), new HashMap<>(), "tests_passed", new HashMap<>()
                ))
                .addStep(new PetriIntentSpec.IntentStep(
                        "alert", PetriIntentSpec.StepType.ACTION, "Send failure alert",
                        Arrays.asList("test_choice"), new HashMap<>(), "tests_failed", new HashMap<>()
                ))
                .build();
    }
    ...
    private PetriIntentSpec createFootballIntentSpec() {
        return PetriIntentSpec.builder()
                ...
                .addStep(new PetriIntentSpec.IntentStep(
                        "parallel_training", PetriIntentSpec.StepType.PARALLEL, "Parallel training activities",
                        Arrays.asList("warmup"), new HashMap<>(), null, new HashMap<>()
                ))
                .addActionStep("pass_practice", "Passing drills")
                .addActionStep("shoot_practice", "Shooting drills")
                .addStep(new PetriIntentSpec.IntentStep(
                        "sync_training", PetriIntentSpec.StepType.SYNC, "Synchronize training completion",
                        Arrays.asList("parallel_training"), new HashMap<>(), null, new HashMap<>()
                ))
                .addStep(new PetriIntentSpec.IntentStep(
                        "cooldown", PetriIntentSpec.StepType.ACTION, "Team cooldown",
                        Arrays.asList("sync_training"), new HashMap<>(), null, new HashMap<>()
                ))
                .build();
    }
}