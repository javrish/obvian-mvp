...
class DemoScenariosIntegrationTest {
    ...
    class DevOpsDeploymentPipelineScenario {
        ...
        void devOpsChoiceStructureForTestPassFailPaths() throws Exception {
            ...
            boolean hasPassPath = petriNet.getTransitions().stream()
                    .anyMatch(t -> t.getDescription() != null &&
                                 t.getDescription().toLowerCase().contains("deploy"));

            boolean hasFailPath = petriNet.getTransitions().stream()
                    .anyMatch(t -> t.getDescription() != null &&
        ...
        void devOpsTokenSimulationThroughSuccessPath() throws Exception {
            ...
            Optional<Transition> runTestsTransition = enabledTransitions.stream()
            ...
            // Fire the transition
            Marking afterTests = petriNet.fireTransition(runTestsTransition.get().getId(), currentMarking);
        ...
        void devOpsConditionalExecutionPathsWorkCorrectly() throws Exception {
            ...
            Optional<Transition> deployTransition = petriNet.getTransitions().stream()
                    .filter(t -> t.getDescription().toLowerCase().contains("deploy"))
                    .findFirst();

            // Find alert transition (failure path)
            Optional<Transition> alertTransition = petriNet.getTransitions().stream()
            ...
            boolean deployHasCondition = deploy.hasGuard() ||
                                       "tests_passed".equals(deploy.getMetadata().get("pathDescription"));
            boolean alertHasCondition = alert.hasGuard() ||
                                      "tests_failed".equals(alert.getMetadata().get("pathDescription"));
        ...
        void devOpsTransformationStatisticsAreReasonable() {
            ...
            AutomationGrammar.TransformationStats stats = automationGrammar.getTransformationStats(devOpsIntent);
    ...
    class FootballTrainingSessionScenario {
        ...
        void footballSequentialWarmupAndCooldownPhases() throws Exception {
            ...
            boolean hasWarmup = petriNet.getTransitions().stream()
                    .anyMatch(t -> t.getDescription() != null &&
                                 t.getDescription().toLowerCase().contains("warm"));

            boolean hasCooldown = petriNet.getTransitions().stream()
                    .anyMatch(t -> t.getDescription() != null &&
        ...
        void footballParallelBranchesCanExecuteIndependently() throws Exception {
            ...
            Optional<Transition> forkTransition = petriNet.getTransitions().stream()
                    .filter(Transition::isFork)
                    .findFirst();
        ...
        void footballTransformationHandlesComplexFlowCorrectly() {
            ...
            AutomationGrammar.TransformationStats stats = automationGrammar.getTransformationStats(footballIntent);
