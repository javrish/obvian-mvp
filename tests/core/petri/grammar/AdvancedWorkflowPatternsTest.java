...
class AdvancedWorkflowPatternsTest {
    ...
    class EnterpriseCiCdTests {
        ...
        void testComplexMicroservicePipeline() throws AutomationGrammar.GrammarTransformationException {
            ...
            // if integration passes, deploy to staging; after staging validation, get approval and deploy to production"

            PetriIntentSpec complexCiCdSpec = PetriIntentSpec.builder()
                    .name("Complex Microservice CI/CD Pipeline")
                    .description("Advanced CI/CD with loops, conditionals, error handling, and approvals")
                    .originalPrompt("For each microservice, run unit tests in parallel; if all pass, run integration tests; if integration passes, deploy to staging; after staging validation, get approval and deploy to production")

                    // 1. Loop through microservices
                    .addLoopStep("foreach_microservice",
                    ...
                    .addNestedConditionalStep("check_unit_tests",
                                            "Evaluate if all unit tests passed",
                                            Map.of("allTestsPassed", "unit_tests.allPassed",
                                                  "coverageThreshold", "unit_tests.coverage >= 80",
                                                  "qualityGate", "unit_tests.qualityGate == 'PASSED'"))
                    ...
                    .addErrorHandlerStep("test_failure_handler",
                    ...
                    .addCircuitBreakerStep("deploy_staging",
                    ...
                    .addEventTriggerStep("approval_gate",
                                       "Wait for production deployment approval",
                                       Map.of("type", "webhook", "approvers", Arrays.asList("team_lead", "devops_manager")))

                    // 9. Production deployment with resource constraints
                    .addResourceConstrainedStep("deploy_production",
            ...
            List<Place> errorPlaces = petriNet.getPlaces().stream()
                    .filter(place -> place.getMetadata().containsKey("stepType") &&
                            "ERROR_HANDLER".equals(place.getMetadata().get("stepType")))
                    .toList();
            assertThat(errorPlaces).isNotEmpty();

            // Verify resource constraints
            List<Place> resourcePools = petriNet.getPlaces().stream()
        ...
        void testMultiStagePipelinePattern() throws AutomationGrammar.GrammarTransformationException {
            PetriIntentSpec pipelineSpec = PetriIntentSpec.builder()
    ...
    class DataPipelineTests {
        ...
        void testDataProcessingPipeline() throws AutomationGrammar.GrammarTransformationException {
            // "Process batch of files: for each file, validate format; if valid, transform and load to database;
            // if any fail, retry once; if still failing, move to error queue and alert data team"

            PetriIntentSpec dataPipelineSpec = PetriIntentSpec.builder()
                    .name("Data Processing Pipeline")
                    .description("Advanced data pipeline with validation, transformation, error handling, and compensation")
                    .originalPrompt("Process batch of files: for each file, validate format; if valid, transform and load to database; if any fail, retry once; if still failing, move to error queue and alert data team")

                    // 1. Loop through files in batch
                    .addLoopStep("foreach_file",
                    ...
                    .addNestedConditionalStep("check_validation",
                                            "Check if file validation passed",
                                            Map.of("formatValid", "validate_format.result == 'VALID'",
                                                  "sizeOk", "validate_format.size <= MAX_FILE_SIZE",
                                                  "schemaMatch", "validate_format.schema == 'EXPECTED'"))
                    ...
                    .addCircuitBreakerStep("load_database",
                    ...
                    .addErrorHandlerStep("processing_error_handler",
                    ...
                    .addCompensationStep("error_queue_handler",
                                       "Handle failed files",
            ...
            List<Place> errorHandlerPlaces = petriNet.getPlaces().stream()
            ...
            List<Transition> compensationTransitions = petriNet.getTransitions().stream()
        ...
        void testRealTimeEventStreaming() throws AutomationGrammar.GrammarTransformationException {
            PetriIntentSpec streamingSpec = PetriIntentSpec.builder()
                    ...
                    .addEventTriggerStep("incoming_events",
                    ...
                    .addResourceConstrainedStep("process_events",
                                              "Process events with limited workers",
                                              Map.of("maxConcurrency", 10,
                                                    "resourceType", "event_processors",
                                                    "queueSize", 1000))
                    ...
                    .addNestedConditionalStep("route_events",
                                            "Route events based on type and priority",
                                            Map.of("highPriority", "event.priority == 'HIGH'",
                                                  "requiresEnrichment", "event.needsEnrichment == true",
                                                  "realTimeProcessing", "event.type == 'REAL_TIME'"))
    ...
    class IncidentResponseTests {
        ...
        void testIncidentResponseWorkflow() throws AutomationGrammar.GrammarTransformationException {
            // "When alert triggers: create incident ticket; notify on-call engineer; if no response in 15 minutes,
            // escalate to team lead; if critical severity, also notify manager immediately"

            PetriIntentSpec incidentSpec = PetriIntentSpec.builder()
                    .name("Incident Response Workflow")
                    .description("Automated incident response with escalation and notifications")
                    .originalPrompt("When alert triggers: create incident ticket; notify on-call engineer; if no response in 15 minutes, escalate to team lead; if critical severity, also notify manager immediately")

                    // 1. Alert trigger event
                    .addEventTriggerStep("alert_trigger",
                    ...
                    .addNestedConditionalStep("severity_check",
                                            "Check incident severity for immediate escalation",
                                            Map.of("isCritical", "alert.severity == 'CRITICAL'",
                                                  "isP1", "alert.priority == 'P1'",
                                                  "affectsProduction", "alert.environment == 'PRODUCTION'"))
            ...
            List<Place> parallelPlaces = petriNet.getPlaces().stream()
            ...
            List<Place> errorHandlingPlaces = petriNet.getPlaces().stream()
                    .filter(place -> place.getMetadata().containsValue("ERROR_HANDLER"))
                    .toList();
        ...
        void testMultiTeamIncidentCoordination() throws AutomationGrammar.GrammarTransformationException {
            PetriIntentSpec coordinationSpec = PetriIntentSpec.builder()
                    .name("Multi-Team Incident Coordination")
                    .description("Coordinate incident response across multiple teams with resource management")

                    // 1. Major incident detection
                    .addEventTriggerStep("major_incident_trigger",
                    ...
                    .addResourceConstrainedStep("form_response_team",
                                              "Form incident response team from available engineers",
                                              Map.of("maxConcurrency", 1,
                                                    "resourceType", "incident_commanders",
                                                    "requiredSkills", Arrays.asList("networking", "database", "frontend")))

                    // 3. Parallel team notifications
                    .addFanOutFanInStep("notify_teams",
                    ...
                    .addTimedStep("coordination_decision",
    ...
    class AdvancedPatternIntegrationTests {
        ...
        void testComplexWorkflowIntegration() throws AutomationGrammar.GrammarTransformationException {
            PetriIntentSpec complexSpec = PetriIntentSpec.builder()
        ...
        void testComplexWorkflowPerformance() throws AutomationGrammar.GrammarTransformationException {
            ...
            for (int i = 0; i < 10; i++) {
                builder.addLoopStep("loop_" + i, "Loop step " + i, "condition_" + i)
                       .addNestedConditionalStep("conditional_" + i, "Conditional step " + i,
                                               Map.of("cond1", "value > " + i, "cond2", "status == 'OK'"))
                       .addCircuitBreakerStep("circuit_" + i, "Circuit breaker " + i,
                                            Map.of("maxRetries", 3))
                       .addResourceConstrainedStep("resource_" + i, "Resource constrained " + i,
                                                  Map.of("maxConcurrency", 2))
                       .addErrorHandlerStep("error_" + i, "Error handler " + i,
                                          Map.of("retry", true), Arrays.asList("compensate_" + i));
            }
            ...
            long optimizationCount = petriNet.getMetadata().entrySet().stream()
