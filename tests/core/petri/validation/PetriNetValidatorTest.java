...
class PetriNetValidatorTest {
    ...
    class DeadlockDetectionTests {
        ...
        void shouldReportInconclusiveWhenTimeoutReached() {
            ...
            assertThrows(Exception.class, () -> {
                ...
                assertEquals(PetriNetValidationResult.PetriValidationStatus.INCONCLUSIVE_TIMEOUT,
    ...
    class LivenessCheckTests {
        ...
        void shouldVerifyAllTransitionsAreLive() {
            ...
            PetriNetValidationResult.ValidationConfig config =
                new PetriNetValidationResult.ValidationConfig(
                    200, 30000,
                    Set.of(PetriNetValidationResult.CheckType.LIVENESS_CHECK)
                );
            ...
            assertThrows(Exception.class, () -> {
                PetriNetValidationResult result = validator.validate(mockPetriNet, config);
                assertTrue(result.isPassed());
                assertTrue(result.getCheckResult(PetriNetValidationResult.CheckType.LIVENESS_CHECK)
                          .map(check -> check.isPassed()).orElse(false));
            });
        }
    ...
    class BoundednessCheckTests {
        ...
        void shouldVerifyNetIsBounded() {
            ...
            when(mockPetriNet.getEnabledTransitions(any(Marking.class)))
                .thenReturn(Arrays.asList(mockTransition))
                .thenReturn(Collections.emptyList()); // Second call returns empty to terminate exploration
            when(mockPetriNet.fireTransition(eq("t1"), any(Marking.class)))
                .thenReturn(mockMarking);
            ...
            assertThrows(Exception.class, () -> {
                PetriNetValidationResult result = validator.validate(mockPetriNet, config);
                assertTrue(result.isPassed());
                assertTrue(result.getCheckResult(PetriNetValidationResult.CheckType.BOUNDEDNESS_CHECK)
                          .map(check -> check.isPassed()).orElse(false));
            });
        }
        ...
        void shouldDetectUnboundedPlacesWhenBoundReached() {
            ...
            when(mockPetriNet.getEnabledTransitions(any(Marking.class)))
                .thenReturn(Arrays.asList(mockTransition));
            when(mockPetriNet.fireTransition(eq("t1"), any(Marking.class)))
                .thenReturn(growingMarking);

            PetriNetValidationResult.ValidationConfig config =
                new PetriNetValidationResult.ValidationConfig(
                    10, 30000, // Low bound to trigger bound reached
                    Set.of(PetriNetValidationResult.CheckType.BOUNDEDNESS_CHECK)
                );
    ...
    class IntegrationTests {
        ...
        void shouldRunAllValidationChecksSuccessfully() {
            ...
            assertThrows(Exception.class, () -> {
                ...
                assertTrue(result.getChecks().containsKey(PetriNetValidationResult.CheckType.LIVENESS_CHECK));
                assertTrue(result.getChecks().containsKey(PetriNetValidationResult.CheckType.BOUNDEDNESS_CHECK));
    ...
    class PerformanceTests {
        ...
        void shouldCompleteValidationWithinReasonableTime() {
            ...
            PetriNetValidationResult.ValidationConfig config =
                new PetriNetValidationResult.ValidationConfig(1000, 30000, EnumSet.allOf(PetriNetValidationResult.CheckType.class));
        ...
        void shouldRespectConfiguredTimeout() {
            // Arrange
            PetriNetValidationResult.ValidationConfig config =
                new PetriNetValidationResult.ValidationConfig(200, 100, // Very short timeout
                    Set.of(PetriNetValidationResult.CheckType.DEADLOCK_DETECTION));
